...
public class HW3 {
	public static void main(String[] args) {
		...
		//printCard (5 points)
		//getSuit (5 points)
		Card newCard=deck.getOneCard();//Card的意思是Card.java裡的public class Card，而newCard是物件的名字。所以整句是newCard這個物件放進deck裡的一張牌
		newCard.printCard();//印出newCard裡的牌
		Card newCard2=deck.getOneCard();//newCard2這個物件放進deck裡的一張牌
		newCard2.printCard();//印出newCard2裡的牌
		deck.shuffle();//deck洗牌
		if(isAllCardsCorrect(deck.getAllCards(),nDeck)){//假如所有牌數都正確
			if(!isShuffleWorking(deck,newCard,newCard2)){//假如洗牌不正確
	...
	private static boolean isShuffleWorking(Deck deck,Card newCard,Card newCard2){
		...
		if(newCard.getSuit().equals(newCard2.getSuit()) &&//假如newCard和newCard2的花色一樣且數字一樣
